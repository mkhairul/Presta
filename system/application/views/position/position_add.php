        ...
        <div id="content">
			...
			<div class="menu">
				<ul>
					...
					<li class="page_item">
						<a href="<?php echo site_url('position/add_position'); ?>" onclick="return show_form('add_new');"><span>Add Position</span></a>
					</li>
				</ul>
			</div>
            ...
            <table border="1" cellspacing="0" cellpadding="0" class="minimal">
				<thead>
					<tr>
						<th class="first">Position Name</th>
						<th>Instances</th>
						<th>Actions</th>
					</tr>
				</thead>
                ...
                <tr>
					...
					<td><?php echo $this->position->get_total_instances($row->id); ?></td>
					<td>
						<a href="<?php echo site_url('position/delete/' . $row->id); ?>" class="delete" onclick="return confirm('Are you sure you want to delete this?')" title="Delete"></a>
					</td>
                </tr>
                <?php
                    }
			...
			<?php if(getstr($this->validation->error_string)){ ?>
				<?php echo form_open('position/add_submit/'); ?>
			<?php } else { ?>
				<?php echo form_open('position/add_submit/', array('class' => 'hide')); ?>
			<?php } ?>
			<fieldset class="lightblue-bg" id="add_new">
				...
				<div>
					<label>Position Name<?php echo required(); ?></label><br/>
					<input id="name" class="full text" type="text" value="<?php echo getstr($this->validation->position);?>" name="name"/>
				</div>
